...
import { AREA_FEATURE_CATEGORY_LABELS, getAreaFeatureTargetKey } from '@/lib/area-features';
...
export default function EventStationAssignmentScreen() {
  ...
  const feature = areaFeatures?.find(
    (candidate) =>
      candidate.category === 'pass' && getAreaFeatureTargetKey(candidate) === decodedTargetKey
  );
  const assignment = assignments?.find((candidate) => candidate.targetKey === decodedTargetKey);

  const acceptedMembers = (members ?? []).filter(
    (member) => member.status === 'accepted' && member.user
  );

  const featureNameByTargetKey = (() => {
